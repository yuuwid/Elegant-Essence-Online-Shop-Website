...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('transactions', function (Blueprint $table) {
            ...
            $table->unsignedTinyInteger('id_status_transaction');

            $table->unsignedBigInteger('id_delivery');
            $table->unsignedSmallInteger('id_payment');

            $table->foreign('id_user')->references('id_user')->on('users')->onDelete('set null');
            $table->foreign('id_status_transaction')->references('id_status_transaction')->on('status_transactions')->onDelete('restrict');
